-- HOTFIX: если verify_user_code падает с
--   function public.normalize_phone_ru(text) does not exist
-- значит обновили RPC без создания функции. Выполните ВЕСЬ этот файл целиком.

-- 1) Функция нормализации (обязательно первой)
...
-- 2) RPC с нормализацией (как в 20250321000006_normalize_auth_phone.sql)
...
-- 3) Проверка
SELECT public.normalize_phone_ru('9958489440') AS normalized;
SELECT public.normalize_phone_ru('+7 (995) 848-94-40') AS normalized2;
